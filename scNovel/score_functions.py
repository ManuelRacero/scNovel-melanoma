...
## La función de este código es la detección de muestras 
## fuera de distribución (ODD). Son varias funciones.

## Normaliza las salidas del modelo
...
## Método ODIN, agrega perturbaciones y ajusta la temp a las salidas del modelo.
...
# Similar a ODIN, pero aplica perturbaciones adicionales y calcula las salidas modificadas para mejorar la detección OOD.
...
#Similar a SEQ, pero con una ligera variación en la forma en que se combinan las puntuaciones originales y perturbadas.
...
# Dependiendo del score_function seleccionado, se aplican las técnicas SIM, SEQ, ODIN o msp para obtener las puntuaciones OOD. Cada método introduce perturbaciones y recalcula las salidas del modelo para identificar ejemplos OOD.
...
def get_ood_scores(loader, net,score_function="sim",T=1000,noise=0.0014,device=torch.device('cpu')):
    ...
    return concat(_score).copy() ##Se agregan puntuaciones calculadas de cada lote 
                                 ## de datos y se concatenan al final
